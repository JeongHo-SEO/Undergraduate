# 🎓 Undergraduate Coursework Archive

Welcome to my undergraduate coursework repository.  
This repository serves as a personal archive for the source codes, assignments, reports, and projects I have completed in my undergraduate period. [2020.02-2027.01]

It also provides overviews and structural roadmaps of my **Student-Designed Track**s decided by council:

> [!NOTE]  
> - Computational Statistics [Bachelor of Science]
> - Financial Data Science [Bachelor of Engineering, Pending Approval]

## 🌐 Academic Exchange Program

I mainly studied Artificial Intelligence on [Daegu Gyeongbuk Institute of Science and Technology](https://www.dgist.ac.kr/), simply **DGIST**.

However, since DGIST focuses on the engineering, I cannot study more for Nature Science such as statistics.  
To fulfill my interest, I completed several specialized courses at partner universities that were not available in the standard DGIST curriculum.

| Exchange Period | Institution | Program / Department | Note |
| :----: | :---------: | :------------------: | :---: |
| 2025-fall | **[🇫🇷 Institut polytechnique de Grenoble - Université Grenoble Alpes (Grenoble INP-UGA)](https://www.grenoble-inp.fr/)** | * [Master of Applied Mathematics (M1-AM)](https://applied-math-master.imag.fr/) <br>* [Master of Science in Industrial and Applied Mathematics (M2-SIAM)](https://msiam.imag.fr/) <br>@ [Ensimag School](https://ensimag.grenoble-inp.fr/) | Master's Program |
| 2025-spring | **[🇰🇷 Seoul National University (SNU)](https://www.snu.ac.kr/)** | [Department of Statistics](https://stat.snu.ac.kr/) | Undergraduate |
| 2022-summer | **[🇰🇷 Seoul National University (SNU)](https://www.snu.ac.kr/)** | [Department of Mathematical Sciences](https://www.math.snu.ac.kr/) | Undergraduate |
| 2021-winter | **[🇰🇷 Seoul National University (SNU)](https://www.snu.ac.kr/)** | [Department of Economics](https://econ.snu.ac.kr/) | Undergraduate |

- In the ***SNU***, I took the **Undergraudate-level statistics courses** to build a solid foundation in statistics.
- In the ***Grenoble INP-UGA***, I took the **Graduate-level statistics courses** to enhance the statistics' structure.

Morevoer, I participated the *General Online Courses* Program which supplies academic credits for [edX](https://www.edx.org/) and [Coursera](https://www.coursera.org/) courses including certificates.

| Period | Institution |
| :----: | :---------: |
| 2026-summer | Coursera |
| 2025-winter | Coursera |
| 2025-summer | edX |

From the online courses, I mainly supplemented several statistics topics which I could not take in my university.  
Moreover, I learned several finance topics from the Coursera.

## Hierarchy

Here is the structure of this repository.

```text
Undergraduate/
├── 🏫 DGIST/
│   └── 📁 AI/
│       ├── course1/
│       └── course2/
├── 🏫 SNU/
│   └── 📁 Stat/
│       ├── course1/
│       └── course2/
├── 🏫 Grenoble INP - UGA/
│   └── 📁 Stat/
│       ├── course1/
│       └── course2/
└── 🖥️ Online Courses/
    ├── edX/
    |    ├── course1/
    |    └── course2/
    └── Coursera/
          ├── course1/
          └── course2/
```

## 🌳 Course-work
The coursework is organized into three foundational pillars (Stat, AI and Math) that converge into advanced, high-level domains.

### Statistics

1. [DGIST-BS202] Introduction to Probability and Mathematical Statistics with R
2. [SNU-326.311 001] Mathematical Statistics l
3. [edX] LSE: Statistics 2 Part 2: Statistical Inference
4. [SNU-326.313 001] Regression Analysis and Lab
5. [SNU-326.413 001] Data Mining Methods and Lab
6. [SNU-M1399.000100 001] Computational Statistics
7. [Coursera] Practical Time Series Analysis
8. [Coursera] Bayesian Statistics: Techniques and Models
9. [Coursera] Bayesian Statistics: Mixture Models
10. [Grenoble INP-UGA-WMM7AM08] Applied probability and statistics
11. [Grenoble INP-UGA-WMM9AM77] Learning, probabilities and causality

### Data Science

1. [DGIST-BE202] Introduction to Data Science
2. [DGIST-BE201] Artificial Intelligence Basics
3. [DGIST-MECH307] Introduction to Artificial Intelligence

### CS

1. [DGIST-BE101a] Introduction to Programming
2. [DGIST-CSE202] Discrete Mathematics
3. [DGIST-CSE201] Object-Oriented Programming
4. [Grenoble INP-UGA-WMM7AM10] Object-Oriented & software design

### Mathematics

1. [DGIST-BS102] Applied Calculus and Differential Equations
2. [DGIST-BS101] Multivariable Calculus
3. [DGIST-BS203] Linear Algebra
4. [SNU-881.003 002] Differential Equations
5. [DGIST-TP309] Introduction to Mathematical Analysis for the Undergraduate
6. [SNU-3341.201 001] Introduction to Mathematical  Analysis l
7. [DGIST-TP310] Modern Algebra

### Finance

1. [DGIST-HSS101] Introduction to Economics
2. [SNU-212.204 001] Introductory Statistics for Economists
3. [Coursera] Risk Management Specialization
4. [Coursera] Credit Risk Modeling & Analysis Mastery Specialization

